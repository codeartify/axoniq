  ...
  template: `<div class="mt-8 bg-blue-50 border border-blue-200 rounded-lg p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      @for (useCase of commonUseCases(); track useCase.route) {
        <button class="bg-white rounded-lg shadow p-6 cursor-pointer hover:shadow-md transition-shadow border border-blue-100 flex items-center gap-4"
          (click)="navigateTo(useCase.route)">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-8 h-8 text-blue-600 flex-shrink-0">
